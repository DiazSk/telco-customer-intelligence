...
class ChurnModelingPipeline:
    ...
    def create_business_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['profitability_score'] = (
            df['TotalCharges'] - 
            (df['tenure'] * df['MonthlyCharges'] * 0.7)  # Assume 70% cost
        )
    ...
    def train_models(self, df: pd.DataFrame) -> Dict:
        ...
        df_model = pd.get_dummies(df[feature_cols + ['churn_binary']], 
                                  drop_first=True)
    ...
    def calculate_business_metrics(self, y_true, y_pred_proba, X_test, df_test):
        ...
        for threshold in thresholds:
            ...
            profit = (
                tp * self.business_params['avg_customer_lifetime_value'] * 
        ...
        return {
            ...
            'precision_at_optimal': np.sum((y_true == 1) & (y_pred_optimal == 1)) / np.sum(y_pred_optimal) if np.sum(y_pred_optimal) > 0 else 0,
            'recall_at_optimal': np.sum((y_true == 1) & (y_pred_optimal == 1)) / np.sum(y_true) if np.sum(y_true) > 0 else 0,
            'customers_to_target': np.sum(y_pred_optimal),
            'expected_saves': np.sum((y_true == 1) & (y_pred_optimal == 1)) * self.business_params['intervention_success_rate']
        }
    ...
    def create_actionable_segments(self, df: pd.DataFrame, model) -> pd.DataFrame:
        ...
        segment_summary['potential_revenue_loss'] = (
            ...
            segment_summary['MonthlyCharges'] * 12 * 
