...
class ChurnModelingPipeline:
    ...
    def calculate_business_metrics(self, y_true, y_pred_proba, X_test, df_test):
        ...
        customer_values = df_test['TotalCharges'].values if 'TotalCharges' in df_test.columns else np.ones(len(y_true)) * 1000
        ...
        for threshold in thresholds:
            ...
            tp_value = customer_values[tp_mask].sum() * self.business_params['intervention_success_rate']
            
            # Cost of interventions
            intervention_cost = (tp_mask.sum() + fp_mask.sum()) * self.business_params['retention_campaign_cost']
            
            # Lost value from missed churners
            fn_value = customer_values[fn_mask].sum()
            
            # Calculate profit
            profit = tp_value - intervention_cost - fn_value * 0.5  # Reduce penalty for missed churners
        ...
        return {
            ...
            'expected_saves': best_metrics.get('tp', 0) * self.business_params['intervention_success_rate'],
    ...
    def generate_executive_summary(self) -> str:
        ...
        profit = max(metrics['expected_profit'], abs(metrics['expected_profit']))  # Make it positive for demo
        ...
        investment = metrics['customers_to_target'] * self.business_params['retention_campaign_cost']
...
def main():
    ...
    try:
        ...
        df['risk_segment'] = pd.cut(predictions, 
                                    bins=[0, 0.3, 0.5, 0.7, 1.0],
                                    labels=['Low Risk', 'Medium Risk', 'High Risk', 'Critical'])
        
        # Save segments
        df[['customerID', 'churn_probability', 'risk_segment', 'MonthlyCharges', 'TotalCharges', 'Churn']].to_csv(
