...
from src.models.advanced_modeling import ChurnModelingPipeline  # noqa: E402
...
class MLflowModelTrainer:
    ...
    def train_production_model(
        self, data_path: str = "data/processed/processed_telco_data.csv"
    ):
        ...
        with mlflow.start_run(
            run_name=f"Production_XGBoost_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ):
    ...
    def create_sample_data(self):
        ...
        churn_prob = (
            0.1
            + 0.3 * (df["Contract"] == "Month-to-month")  # Base rate
            + 0.2 * (df["tenure"] < 12)
